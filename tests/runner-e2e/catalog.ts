  ...
  },
  {
    ...
    buildPrompt: (nonce) =>
      [
        ...
        `In a native runner, in the same provider turn first emit exactly E2E_ASK_12_${nonce} as the complete final response, then call paperclip_finish exactly once with {reportedWorkDisposition:"done",summary:"E2E_ASK_12_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Do not wait for the terminal tool result before emitting the response, and do not call another tool.`,
...
const structuredQuestionResumeTask = {
  ...
  buildPrompt: (nonce) =>
    [
      ...
      `In a native runner, after the answer arrives, in the same provider turn first emit exactly PAPERCLIP_E2E_QUESTION_DONE_${nonce} as the complete final response, then call paperclip_finish exactly once with {reportedWorkDisposition:"done",summary:"PAPERCLIP_E2E_QUESTION_DONE_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Do not wait for the terminal tool result before emitting the response, and do not call another tool.`,
